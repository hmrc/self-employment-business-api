...
import v5.createAmendAnnualSubmission.model.request.Def4_CreateAmendAnnualSubmissionRequestData

import scala.util.matching.Regex

object Def4_CreateAmendAnnualSubmissionRulesValidator extends RulesValidator[Def4_CreateAmendAnnualSubmissionRequestData] {
  ...
  def validateBusinessRules(
      parsed: Def4_CreateAmendAnnualSubmissionRequestData): Validated[Seq[MtdError], Def4_CreateAmendAnnualSubmissionRequestData] = {
    import parsed.body.*
    combine(
  ...
  private def validateAdjustments(adjustments: request.Def4_CreateAmend_Adjustments): Validated[Seq[MtdError], Unit] = {
    import adjustments.*
    ...
    val validatedNonNegatives = List(
      ...
      (adjustmentToProfitsForClass4, "/adjustments/adjustmentToProfitsForClass4")
  ...
  private def validateAllowances(allowances: request.Def4_CreateAmend_Allowances): Validated[Seq[MtdError], Unit] = {
    import allowances.*
  ...
  private def validateStructuredBuildingAllowance(structuredBuildingAllowance: request.Def4_CreateAmend_StructuredBuildingAllowance,
                                                  index: Int,
  ...
  private def validateBuildingNameNumber(building: request.Def4_CreateAmend_Building, path: String): Validated[Seq[MtdError], Unit] = {
    building match {
      case request.Def4_CreateAmend_Building(None, None, _) => Invalid(List(RuleBuildingNameNumberError.withPath(path)))
      case _                                                => valid
    }
  }

  private def validateTPAAmount(adjustments: request.Def4_CreateAmend_Adjustments): Validated[Seq[MtdError], Unit] = {
