...
import v1.models.request.createSEPeriodic.{ConsolidatedExpenses, CreateSelfEmploymentPeriodicBody, CreateSelfEmploymentPeriodicRawData, Expenses, Incomes}

class CreateSelfEmploymentPeriodicValidator extends Validator[CreateSelfEmploymentPeriodicRawData] {
  ...
  private def parameterFormatValidation: CreateSelfEmploymentPeriodicRawData => List[List[MtdError]] = (data: CreateSelfEmploymentPeriodicRawData) => {
    List(
      NinoValidation.validate(data.nino),
      BusinessIdValidation.validate(data.businessId),
      JsonFormatValidation.validate[CreateSelfEmploymentPeriodicBody](data.body)
    )
  }

  private def incorrectOrEmptyBodySubmittedValidation: CreateSelfEmploymentPeriodicRawData => List[List[MtdError]] = { data =>
    val body = data.body.as[CreateSelfEmploymentPeriodicBody]
    if (body.isEmpty) List(List(RuleIncorrectOrEmptyBodyError)) else NoValidationErrors
  }

  private def consolidatedExpensesRuleValidation: CreateSelfEmploymentPeriodicRawData => List[List[MtdError]] = (data: CreateSelfEmploymentPeriodicRawData) => {
    val body = data.body.as[CreateSelfEmploymentPeriodicBody]
  ...
  private def bodyFieldValidation: CreateSelfEmploymentPeriodicRawData => List[List[MtdError]] = { data =>
    val body = data.body.as[CreateSelfEmploymentPeriodicBody]
  ...
  private def dateRuleValidation: CreateSelfEmploymentPeriodicRawData => List[List[MtdError]] = (data: CreateSelfEmploymentPeriodicRawData) => {
    val body = data.body.as[CreateSelfEmploymentPeriodicBody]
    List(
  ...
  override def validate(data: CreateSelfEmploymentPeriodicRawData): List[MtdError] = {
