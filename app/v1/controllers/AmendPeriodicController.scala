...
import v1.services.{ AmendPeriodicService, EnrolmentsAuthService, MtdIdLookupService }

import javax.inject.{ Inject, Singleton }
import scala.concurrent.{ ExecutionContext, Future }

@Singleton
class AmendPeriodicController @Inject()(val authService: EnrolmentsAuthService,
                                        val lookupService: MtdIdLookupService,
                                        parser: AmendPeriodicRequestParser,
                                        service: AmendPeriodicService,
  ...
  implicit val endpointLogContext: EndpointLogContext =
    EndpointLogContext(controllerName = "AmendPeriodicController", endpointName = "amendSelfEmploymentPeriodicSummary")
      ...
      val result =
        for {
          ...
          vendorResponse <- EitherT.fromEither[Future](
            hateoasFactory
              .wrap(serviceResponse.responseData, AmendPeriodicHateoasData(nino, businessId, periodId))
