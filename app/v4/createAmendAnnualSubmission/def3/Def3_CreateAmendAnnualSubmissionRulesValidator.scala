...
import v4.createAmendAnnualSubmission.model.request.Def3_CreateAmendAnnualSubmissionRequestData
...
object Def3_CreateAmendAnnualSubmissionRulesValidator extends RulesValidator[Def3_CreateAmendAnnualSubmissionRequestData] {
  ...
  private def validateBuildingNameNumber(building: request.Def3_CreateAmend_Building, path: String): Validated[Seq[MtdError], Unit] = {
    building match {
      case request.Def3_CreateAmend_Building(None, None, _) => Invalid(List(RuleBuildingNameNumberError.withPath(path)))
      case _                                                => valid
    }
  }

  private def validateTPAAmount(adjustments: request.Def3_CreateAmend_Adjustments): Validated[Seq[MtdError], Unit] = {
