...
//    type DownstreamResp = Def2_RetrieveAnnualSubmissionResponse
    val connectorReads: Reads[DownstreamResp] = null // Def2_RetrieveAnnualSubmissionResponse.reads
  }
  ...
  def schemaFor(taxYear: TaxYear): RetrieveAnnualSubmissionSchema =
    if (taxYear <= TaxYear.starting(2022)) Def1
    else if (taxYear == TaxYear.starting(2023)) Def2
    else latestSchema
