  ...
  "reads" should {
    "passed valid mtd JSON" should {
      "return the model" in {
        ...
        requestJson.as[Def4_CreateAmend_NonFinancials] shouldBe Def4_CreateAmend_NonFinancials(
    ...
    "writes" when {
      "there is an exemption reason" must {
        "set exemptFromPayingClass4Nics true" in {
          Json.toJson(Def4_CreateAmend_NonFinancials(class4NicsExemptionReason = Some(MtdNicExemption.`non-resident`))) shouldBe
            Json.parse(s"""
                 |{
      ...
      "there is no exemption reason" must {
        "set exemptFromPayingClass4Nics false" in {
          Json.toJson(Def4_CreateAmend_NonFinancials(class4NicsExemptionReason = None)) shouldBe
            Json.parse(s"""
                 |{
