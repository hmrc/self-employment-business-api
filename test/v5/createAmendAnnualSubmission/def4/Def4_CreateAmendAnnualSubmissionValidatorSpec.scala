...
import v5.createAmendAnnualSubmission.def4.request.*
import v5.createAmendAnnualSubmission.model.request.*

class Def4_CreateAmendAnnualSubmissionValidatorSpec extends UnitSpec with JsonErrorValidators {
  ...
  private val parsedAdjustments = Def4_CreateAmend_Adjustments(
    ...
    adjustmentToProfitsForClass4 = Some(200.12)
  )

  private val parsedFirstYear = Def4_CreateAmend_FirstYear(qualifyingDate = "2021-11-11", qualifyingAmountExpenditure = 1.23)
  private val parsedBuilding  = Def4_CreateAmend_Building(name = Some("Plaza 2"), number = None, postcode = "TF3 4NT")

  private val parsedStructuredBuildingAllowance =
    Def4_CreateAmend_StructuredBuildingAllowance(amount = 1.23, firstYear = Some(parsedFirstYear), building = parsedBuilding)

  private val parsedAllowances = Def4_CreateAmend_Allowances(
  ...
  private val parsedNonFinancials =
    Def4_CreateAmend_NonFinancials(class4NicsExemptionReason = Some(MtdNicExemption.parser("non-resident")))

  private val parsedRequestBody =
    Def4_CreateAmendAnnualSubmissionRequestBody(Some(parsedAdjustments), Some(parsedAllowances), Some(parsedNonFinancials))
  ...
  "validate()" should {
    "return the parsed domain object" when {
      "a valid request is made" in {
        ...
        result shouldBe Right(
          Def4_CreateAmendAnnualSubmissionRequestData(parsedNino, parsedBusinessId, parsedTaxYear, parsedRequestBody)
        )
      }
      ...
      "a minimal adjustments request is supplied" in {
        ...
        val expected = Def4_CreateAmendAnnualSubmissionRequestData(
          parsedNino,
          parsedBusinessId,
          parsedTaxYear,
      ...
      "only allowances is supplied" in {
        ...
        val result: Either[ErrorWrapper, CreateAmendAnnualSubmissionRequestData] =
          validator(validNino, validBusinessId, validTaxYear, requestBody).validateAndWrapResult()
        result shouldBe Right(
          Def4_CreateAmendAnnualSubmissionRequestData(
            parsedNino,
            parsedBusinessId,
            parsedTaxYear,
    ...
    List(
      ...
      "/adjustments/adjustmentToProfitsForClass4",
        ...
        val result: Either[ErrorWrapper, CreateAmendAnnualSubmissionRequestData] = validator(
          ...
          Json.parse(
            """
              ...
              |    "adjustmentToProfitsForClass4": 200.132
              |  },
              |  "allowances": {
