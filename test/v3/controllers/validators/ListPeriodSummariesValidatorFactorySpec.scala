  ...
  "validator" should {
    "return the parsed domain object" when {
      "a valid request is made" in {
        ...
        result shouldBe Right(ListPeriodSummariesRequestData(parsedNino, parsedBusinessId, parsedTaxYear))
      }
      "a valid request with no tax year supplied" in {
        val result: Either[ErrorWrapper, ListPeriodSummariesRequestData] =
          validator(validNino, validBusinessId, None).validateAndWrapResult()

        result shouldBe Right(ListPeriodSummariesRequestData(parsedNino, parsedBusinessId, None))
      }
    }
    ...
    "return BusinessIdFormatError error" when {
      "an invalid nino is supplied" in {
        val result: Either[ErrorWrapper, ListPeriodSummariesRequestData] =
          validator(validNino, "invalid", Some(validTaxYear)).validateAndWrapResult()
    ...
    "return TaxYearFormat" when {
      "an invalid tax year is supplied" in {
        val result: Either[ErrorWrapper, ListPeriodSummariesRequestData] =
          validator(validNino, validBusinessId, Some("invalid")).validateAndWrapResult()
    ...
    "return multiple errors" when {
      "request supplied has multiple errors" in {
        val result: Either[ErrorWrapper, ListPeriodSummariesRequestData] = validator("invalid", "invalid", Some("invalid")).validateAndWrapResult()
