...
class Def2_CreateAmend_NonFinancialsSpec extends UnitSpec {
  ...
  "reads" should {
    "passed valid mtd JSON" should {
      "return the model" in {
        ...
        requestJson.as[Def2_CreateAmend_NonFinancials] shouldBe Def2_CreateAmend_NonFinancials(
          businessDetailsChangedRecently = true,
    ...
    "writes" when {
      "there is an exemption reason" must {
        "set exemptFromPayingClass4Nics false" in {
          Json.toJson(
            Def2_CreateAmend_NonFinancials(businessDetailsChangedRecently = true, class4NicsExemptionReason = Some(MtdNicExemption.`non-resident`))) shouldBe
            Json.parse(s"""
                 |{
                 |  "businessDetailsChangedRecently": true,
      ...
      "there is no exemption reason" must {
        "set exemptFromPayingClass4Nics true" in {
            Json.toJson(Def2_CreateAmend_NonFinancials(businessDetailsChangedRecently = true, class4NicsExemptionReason = None)) shouldBe
            Json.parse(s"""
                 |{
                 |  "businessDetailsChangedRecently": true,
