...
package v3.models.response.listPeriodSummaries
  ...
  "LinksFactory" when {
    ...
    "TYS feature switch is disabled" should {
      "return the correct top-level links" in {
        ...
        ListPeriodSummariesResponse.LinksFactory.links(mockAppConfig, hateoasData) shouldBe List(
    ...
    "TYS feature switch is enabled and tax year is TYS" should {
      "return the correct top-level links" in {
        ...
        ListPeriodSummariesResponse.LinksFactory.links(mockAppConfig, hateoasDataTys) shouldBe List(
          Link(href = s"/test/context/$nino/$businessId/period", method = POST, rel = "create-self-employment-period-summary"),
