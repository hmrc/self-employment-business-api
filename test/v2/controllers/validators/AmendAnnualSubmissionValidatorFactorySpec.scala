...
class AmendAnnualSubmissionValidatorFactorySpec extends UnitSpec with JsonErrorValidators with MockSeBusinessConfig {
  ...
  private val parsedAdjustments = Adjustments(
  ...
  private val parsedFirstYear = FirstYear(qualifyingDate = "2021-11-11", qualifyingAmountExpenditure = 1.23)
  private val parsedBuilding  = Building(name = Some("Plaza 2"), number = None, postcode = "TF3 4NT")

  private val parsedStructuredBuildingAllowance =
    StructuredBuildingAllowance(amount = 1.23, firstYear = Some(parsedFirstYear), building = parsedBuilding)

  private val parsedAllowances = Allowances(
  ...
  private val parsedAllowancesWithOnlyTradingIncomeAllowance =
    Allowances(None, None, None, None, None, None, None, None, tradingIncomeAllowance = Some(200.12), None, None, None, None)

  private val parsedNonFinancials =
    NonFinancials(businessDetailsChangedRecently = true, class4NicsExemptionReason = Some(MtdNicExemption.parser("non-resident")))

  private val parsedRequestBody = AmendAnnualSubmissionBody(Some(parsedAdjustments), Some(parsedAllowances), Some(parsedNonFinancials))

  private val validatorFactory = new AmendAnnualSubmissionValidatorFactory
  ...
  "validator" should {
    "return the parsed domain object" when {
      "a valid request is made" in {
        ...
        result shouldBe Right(
          AmendAnnualSubmissionRequestData(parsedNino, parsedBusinessId, parsedTaxYear, parsedRequestBody)
        )
      }
      ...
      "a minimal adjustments request is supplied" in {
        ...
        result shouldBe Right(
          AmendAnnualSubmissionRequestData(
            parsedNino,
            parsedBusinessId,
            parsedTaxYear,
    ...
    "return multiple errors" when {
      ...
      "every field in the body is invalid" in {
        val result: Either[ErrorWrapper, AmendAnnualSubmissionRequestData] = validator(
          validNino,
          validBusinessId,
          validTaxYear,
          Json.parse(
