...
class Def1_CreatePeriodSummaryValidatorSpec extends UnitSpec with JsonErrorValidators with MockSharedAppConfig {
  ...
  private def parsedBody(periodDates: PeriodDates = parsedPeriodDates,
    ...
    Def1_CreateAmendCumulativeSubmissionRequestBody(periodDates, periodIncome, periodExpenses, periodDisallowableExpenses)

  private def validator(nino: String, businessId: String, taxYear: TaxYear, body: JsValue): Def1_CreateAmendCumulativeSubmissionValidator =
    new Def1_CreateAmendCumulativeSubmissionValidator(nino, businessId, taxYear, body)
  ...
  "validator" should {
    "return the parsed domain object" when {
      "given a valid request" in {
        ...
        val result: Either[ErrorWrapper, CreateAmendCumulativeSubmissionRequestData] =
          validator(validNino, validBusinessId, taxYear, validBody())
            .validateAndWrapResult()

        result shouldBe Right(
          Def1_CreateAmendCumulativeSubmissionRequestData(parsedNino, parsedBusinessId, taxYear, parsedBody())
        )
      }
      ...
      "given a valid request a body without period disallowable expenses" in {
        ...
        result shouldBe Right(
          Def1_CreateAmendCumulativeSubmissionRequestData(parsedNino, parsedBusinessId, taxYear, parsedBody(periodDisallowableExpenses = None))
        )
      }
      ...
      "given a valid request a body without period allowable expenses" in {
        ...
        result shouldBe Right(
          Def1_CreateAmendCumulativeSubmissionRequestData(parsedNino, parsedBusinessId, taxYear, parsedBody(periodExpenses = None))
        )
      }
    }
