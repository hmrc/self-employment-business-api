...
class CreatePeriodValidatorSpec extends UnitSpec {
  ...
  val validator = new CreatePeriodicValidator()

  "running a validation" should {
    "return no errors" when {
      "a valid expenses request is supplied" in {
        validator.validate(CreatePeriodicRawData(validNino, validBusinessId, requestBodyExpensesJson)) shouldBe Nil
      }

      "a valid consolidated expenses request is supplied" in {
        validator.validate(CreatePeriodicRawData(validNino, validBusinessId, requestBodyConsolidatedExpensesJson)) shouldBe Nil
      }

      "the minimum fields are supplied" in {
        validator.validate(CreatePeriodicRawData(validNino, validBusinessId, Json.parse(
          """
            |{
            |   "periodFromDate": "2017-01-25",
            |   "periodToDate": "2018-01-24"
    ...
    "return a path parameter error" when {
      "an invalid nino is supplied" in {
        validator.validate(CreatePeriodicRawData("walrus", validBusinessId, requestBodyExpensesJson)) shouldBe List(NinoFormatError)
      }

      "an invalid businessId is supplied" in {
        validator.validate(CreatePeriodicRawData(validNino, "beans", requestBodyExpensesJson)) shouldBe List(BusinessIdFormatError)
      }
    }
    ...
    "return all errors" when {
      "all path parameters are invalid" in {
        validator.validate(CreatePeriodicRawData("walrus", "beans", requestBodyExpensesJson)) shouldBe
